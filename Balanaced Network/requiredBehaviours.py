...
class iSTDP(Behavior):
    ...
    def get_weight_changes(self, s):
        pre_spike_changes = self.lr * (torch.outer(s.dst.trace - self.alpha, s.src.spikes)) 
...
class Homeostasis(Behavior):
    ...
    @staticmethod
    def init_exhaustion(neurons):
        neurons.exhaustion = neurons.vector(mode="zeros")

    def initialize(self, neurons):
        self.add_tag('Homeostasis')
        self.max_v = self.parameter('max_v', None)
        self.min_v = self.parameter('min_v', None)
        self.adj_strength = self.parameter('eta_ip', 0.001)


    def forward(self, neurons):
        self.init_exhaustion(neurons)
        greater = ((neurons.v > self.max_v) * -1).type(torch.float32)
        smaller = (neurons.v < self.min_v).type(torch.float32)

        greater *= neurons.v - self.max_v
        smaller *= self.min_v - neurons.v
